...
///
/// ★IT IS ALSO THE SHIPPING [VoiceHost]. This class is the Dart handle on the
/// whole `ai.bithuman.avatar` channel, and that channel is two modules: 12
/// render-only verbs, 11 voice-only ones and 4 shared (ARCHITECTURE.md, "which
/// half is voice"). The conformance declares the voice half as a surface the
/// voice module can name WITHOUT naming this class — it adds no code, because
/// every member of [VoiceHost] already existed here with these signatures. The
/// arrow runs render → voice: `lib/bithuman_realtime.dart` and
/// `lib/realtime_transport.dart` no longer import this file, and
/// `scripts/check_voice_render_edge_dart.sh` keeps it that way.
class BithumanAvatar implements VoiceHost {
